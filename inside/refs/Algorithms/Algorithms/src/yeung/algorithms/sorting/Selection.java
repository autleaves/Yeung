...
public class Selection {
	 ...
	 * a.length; for (int i = 0; i < n; i++) { int min = i; for (int j = i+1; j < n;
	 * j++) { if (less(comparator, a[j], a[min])) min = j; } exch(a, i, min); assert
	 * isSorted(a, comparator, 0, i); } assert isSorted(a, comparator); }
	 */
    ...
    ***************************************************************************/
    ...
    // is the array a[] sorted?
	/*
	 * private static boolean isSorted(Object[] a, Comparator comparator) { return
	 * isSorted(a, comparator, 0, a.length - 1); }
	 */

    // is the array sorted from a[lo] to a[hi]
	/*
	 * private static boolean isSorted(Object[] a, Comparator comparator, int lo,
	 * int hi) { for (int i = lo + 1; i <= hi; i++) if (less(comparator, a[i],
	 * a[i-1])) return false; return true; }
	 */
