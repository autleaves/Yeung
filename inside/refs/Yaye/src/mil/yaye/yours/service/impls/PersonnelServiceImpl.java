...
public class PersonnelServiceImpl implements PersonnelService {
	...
	//Fields
	private static Logger logger = Logger.getLogger(PersonnelServiceImpl.class.getName());
	...
	public boolean exit(String logonid){
		...
		UsersDAO user_dao = this.user_dao; //为了防止各个线程之间的资源争夺,我在每个方法内再定义一些那些公用属性的引用
		try {
...
//			user.setUserId(id);
//			tran.commit();
			logger.info("新插入的一条user表的记录id为:" + id+ "########");
